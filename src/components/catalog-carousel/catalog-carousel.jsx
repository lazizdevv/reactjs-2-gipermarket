    ...
    <div className="relative w-full max-w-screen-2xl mx-auto my-8 overflow-hidden">
      ...
      >
        {banners.map((banner, index) => (
          ...
          >
            <div className="bg-accent shadow-lg shadow-primary p-4 w-60 flex flex-col justify-center items-center text-center">
              <img src={banner.img} alt={banner.id} className="w-24 h-auto object-cover" />
      ...
      <Button onClick={prevSlide} className="absolute left-2 lg:left-0 top-1/2 transform -translate-y-1/2 bg-white w-10 h-10 rounded-full flex justify-center">
        <ArrowLeftIcon />
      </Button>
      <Button onClick={nextSlide} className="absolute right-2 lg:right-0 top-1/2 transform -translate-y-1/2 bg-white w-10 h-10 rounded-full flex justify-center">
        <ArrowRightIcon />
      </Button>
